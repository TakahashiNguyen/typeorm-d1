...
describe("D1Guards", () => {
  it("detects valid and invalid D1 database objects", () => {
    ...
    expect(
      D1Guards.isD1Database({ prepare: jest.fn(), batch: jest.fn() }),
    ).toBe(false);
    expect(
      D1Guards.isD1Database({
        prepare: jest.fn(),
        batch: jest.fn(),
        exec: "nope",
      }),
  ...
  it("asserts driver and connection state", () => {
    expect(() => D1Guards.assertDriverInitialized(undefined)).toThrow(
      "Driver is not initialized",
    );
    expect(() => D1Guards.assertDriverInitialized({} as any)).not.toThrow();

    expect(() => D1Guards.assertConnectionEstablished(undefined)).toThrow(
      "Database connection is not established",
    );
    expect(() =>
      D1Guards.assertConnectionEstablished(validDatabase),
    ).not.toThrow();
  });
  ...
  it("validates query parameters", () => {
    ...
    expect(() => D1Guards.validateQueryParameters("value" as any)).toThrow(
      "Query parameters must be an array",
    );
  });

  it("asserts required values and strings", () => {
    expect(() => D1Guards.assertNotNull("value", "Value")).not.toThrow();
    expect(() => D1Guards.assertNotNull(null, "Value")).toThrow(
      "Value must not be null or undefined",
    );
    expect(() => D1Guards.assertNotNull(undefined, "Value")).toThrow(
      "Value must not be null or undefined",
    );

    expect(() =>
      D1Guards.assertNonEmptyString("table", "Table name"),
    ).not.toThrow();
    expect(() => D1Guards.assertNonEmptyString("", "Table name")).toThrow(
    ...
    expect(() =>
      D1Guards.assertNonEmptyString(undefined, "Table name"),
...
describe("D1DriverFactory", () => {
  it("throws when driver database is missing", () => {
    ...
    expect(() => factory.create(dataSource)).toThrow(
      "D1DriverFactory requires driver.database",
    );
  });
